...
function formatStaticMonthPeriod(
  period,
  language
) {
  if (
...
function prerenderCzechHomepageSeo(
  html,
  eu
) {
  const czechia =
  ...
  if (
    ...
    typeof interestRate.period !== "string"
  ) {
    throw new Error(
      "Missing Czech ECB interest-rate data"
    );
  }
...
async function prerenderCountrySeo(
  ...
  if (
    ...
    throw new Error(
      `Missing ECB interest-rate SEO data for ${country.code}`
    );
  }
...
const prerenderedHomepage =
  prerenderCzechHomepageSeo(
    homepageSource,
    eu
  );
